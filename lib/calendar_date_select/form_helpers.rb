  ...
  def calendar_date_select(object, method, options={})
    ...
    options[:value] ||=
      if(obj.respond_to?(method) && obj.send(method).respond_to?(:strftime))
		...
		if javascript_options[:hidden] || javascript_options[:embedded]
			tag = ActionView::Helpers::Tags::HiddenField.new(object, method, self, options)
		else
  ...
  private
    ...
    def calendar_date_select_output(input, image, options = {}, javascript_options = {})
      ...
      if javascript_options[:embedded]
        ...
        out << javascript_tag("new CalendarDateSelect( $('#{uniq_id}').previous(), #{javascript_options.to_json} ); ")
      else
        out << " "
        out << image_tag(image,
            :onclick => "new CalendarDateSelect( $(this).previous(), #{javascript_options.to_json} );",
